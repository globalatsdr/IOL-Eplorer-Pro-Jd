...
import { Lens, EquivalentSettings } from '../types';
import { X, ArrowDownAZ, ArrowUpAZ, Sparkles, Download, Loader2, Wrench } from 'lucide-react';
...
interface Props {
  ...
  onFindSimilar: (lens: Lens, manufacturer: string, settings: EquivalentSettings) => void;
...
const ComparisonView: React.FC<Props> = ({ lenses, onClose, onRemove, onFindSimilar, manufacturers }) => {
  ...
  const [eqSettings, setEqSettings] = useState<EquivalentSettings>({
    opticConcept: true,
    toric: true,
    technology: false,
    aberration: false,
  ...
  const toggleEqSetting = (key: keyof EquivalentSettings) => {
    setEqSettings(prev => ({ ...prev, [key]: !prev[key] }));
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                <button 
                  ...
                  className={`p-2 rounded-lg transition-all ${showSettings ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-200 text-gray-500'}`}
                  title="Configure Equivalent Search"
                >
                  <Wrench className="w-5 h-5" />
                </button>

                {showSettings && (
                  <div className="absolute left-0 top-full mt-2 w-64 bg-white rounded-xl shadow-2xl border border-slate-200 z-[60] p-4 animate-in zoom-in-95 duration-200">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Equivalent Search</h3>
                      <Wrench className="w-3 h-3 text-slate-300" />
                    </div>
                    
                    <div className="space-y-3">
                      {[
                        { id: 'opticConcept', label: 'Optic Concept' },
                        { id: 'toric', label: 'Toric' },
                        { id: 'technology', label: 'Technology' },
                        { id: 'aberration', label: 'Aberration' },
                        { id: 'material', label: 'Material' },
                        { id: 'hapticDesign', label: 'Haptic Design' }
                      ].map((item) => (
                        <div key={item.id} className="flex items-center justify-between">
                          <span className="text-sm font-bold text-slate-600">{item.label}</span>
                          <button
                            onClick={() => toggleEqSetting(item.id as keyof EquivalentSettings)}
                            className={`w-10 h-5 rounded-full transition-colors relative ${eqSettings[item.id as keyof EquivalentSettings] ? 'bg-blue-500' : 'bg-slate-200'}`}
                          >
                            <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${eqSettings[item.id as keyof EquivalentSettings] ? 'left-6' : 'left-1'}`} />
                          </button>
                        </div>
                      ))}
                    </div>
                    ...
                    <th key={lens.id} className="p-4 bg-white border-b-2 border-gray-200 min-w-[250px] relative group transition-colors hover:bg-slate-50">
                      ...
                      {!isExporting && (
                        <div className="flex items-center gap-1.5 w-full">
                          ...
                          <select 
                            onChange={(e) => onFindSimilar(lens, e.target.value, eqSettings)}
