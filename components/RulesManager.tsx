...
const RulesManager: React.FC<Props> = ({ rules, onClose }) => {
  ...
  const mapAgeGroupToString = (group?: number[]): string => {
    ...
    return group.map((g: number) => AGE_RANGES[g] || '?').join(', ');
  };

  const mapLaGroupToString = (group?: number[]): string => {
    if (!group || group.length === 0) return 'Cualquiera';
    return group.map((g: number) => LA_RANGES[g] || '?').join(', ');
  };

  const renderSpecialConditions = (rule: Rule) => {
    const required = rule.conditions.specialConditions?.map((key: string) => specialConditionsOptions[key] || key);
    const negated = rule.conditions.negatedConditions?.map((key: string) => specialConditionsOptions[key] || key);
